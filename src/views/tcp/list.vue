        ...
        <v-dialog v-model="addDialogStatus" fullscreen hide-overlay transition="dialog-bottom-transition">
            <v-card>
                <v-toolbar dark class="deep-purple">
                    ...
                    <v-toolbar-title>TCP监控</v-toolbar-title>
                    <v-spacer></v-spacer>
                    <v-toolbar-items>
                        <v-btn dark text @click="createTcp()">
                            确认保存
                        </v-btn>
                    </v-toolbar-items>
                </v-toolbar>
    ...
    export default {
        ...
        methods: {
            ...
            pauseItem(item) {
                ...
                this.$service.tcp.pauseService(formData)
                    .then(() => {
            ...
            startMonitor(item) {
                const formData = new FormData();
                formData.append("id", item.id)
                this.$service.tcp.startMonitorService(formData)
                    .then(() => {
                        this.getTcpList(this.targetGroup);
                    })
            },
            file(item) {
                const formData = new FormData();
                formData.append("id", item.id)
                this.$service.tcp.fileService(formData)
                    .then(() => {
                        this.getTcpList(this.targetGroup);
                    })
            },
            getFiledList() {
                ...
                this.groupType = 2;
                this.$service.tcp.getFiledListService(this.pagination.page, this.pagination.itemsPerPage)
                    .then(res => {
            ...
            createTcp() {
                ...
                formData.append("tcp_id", this.targetDevice);
                ...
                if (this.targetDevice == 0) {
                    this.$service.tcp.createService(formData)
                        .then(res => {
                            if (res["code"] == 200) {
                                this.getTcpList(this.targetGroup);
                                this.errText = "添加成功";
                    ...
                    this.$service.tcp.updateService(formData)
                        .then(res => {
                            if (res["code"] == 200) {
                                this.getTcpList(this.targetGroup);
                                this.errText = "更新成功";
            ...
            getTcpList(groupId) {
                ...
                this.$service.tcp.getTcpListService(groupId, this.pagination.page, this.pagination.itemsPerPage)
                    .then(res => {
                        this.list = res['data'].data != null ? res['data'].data : [];
                        this.pagination.itemsLength = res.data.total;
                        this.pagination.page = res.data.page_index;
            ...
            getGroupList(teamId) {
                this.$service.group.getListByTypeService(teamId, 4)
                    .then(res => {
            ...
            redirectToOverview(item) {
                ...
                this.$router.push(`/user/tcp/${item.id}/overview`);
            },
            deleteDevice(id) {
                this.$service.tcp.deleteService(id)
                    .then(() => {
                        this.getTcpList(this.targetGroup);
                    })
            },
