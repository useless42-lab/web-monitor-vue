        ...
        <v-dialog v-model="addDialogStatus" fullscreen hide-overlay transition="dialog-bottom-transition">
            <v-card>
                <v-toolbar dark class="deep-purple">
                    ...
                    <v-toolbar-title>网站监控</v-toolbar-title>
                    <v-spacer></v-spacer>
                    <v-toolbar-items>
                        <v-btn dark text @click="createWeb()">
                            确认保存
                        </v-btn>
                    </v-toolbar-items>
                </v-toolbar>
                <v-card-text>
                    <v-container>
                        <v-row>
                            ...
                            <v-col cols="12" sm="12">
                                <v-select v-model="urlPrefix" :items="urlPrefixItem" label="网址"></v-select>
                            </v-col>
                            <v-col cols="12" sm="12">
                                <v-text-field v-model="url" label="网址" hide-details="auto"></v-text-field>
                            </v-col>
                            <v-col cols="12" sm="12">
                            ...
                            <v-col cols="12" sm="12">
                                <v-text-field v-model="basicUser" label="Basic Authentication User" hide-details="auto">
                                </v-text-field>
                            </v-col>

                            <v-col cols="12" sm="12">
                                <v-text-field v-model="basicPassword" label="Basic Authentication Password"
    ...
    export default {
        ...
        methods: {
            getDetail(item) {
                this.$service.device.getDetailService(item.id, this.deviceType)
                    .then(res => {
                        ...
                        this.url = res['data'].path.split("//")[1];
                        this.targetPolicy = res['data'].policy_id;
                        this.basicUser = res['data'].basic_user;
                        this.basicPassword = res['data'].basic_password;
            ...
            pauseItem(item) {
                ...
                this.$service.web.pauseService(formData)
                    .then(() => {
                        this.getWebList(this.targetGroup);
                    })
            },
            ...
            startMonitor(item) {
                const formData = new FormData();
                formData.append("id", item.id)
                this.$service.web.startMonitorService(formData)
                    .then(() => {
                        this.getWebList(this.targetGroup);
                    })
            },
            file(item) {
                const formData = new FormData();
                formData.append("id", item.id)
                this.$service.web.fileService(formData)
                    .then(() => {
                        this.getWebList(this.targetGroup);
                    })
            },
            getFiledList() {
                ...
                this.groupType = 2;
                this.$service.web.getFiledListService(this.pagination.page, this.pagination.itemsPerPage)
                    .then(res => {
            ...
            createWeb() {
                ...
                formData.append("path", this.urlPrefix + this.url);
                ...
                formData.append("basic_user", this.basicUser);
                formData.append("basic_password", this.basicPassword);
                formData.append("web_id", this.targetDevice);
                ...
                if (this.targetDevice == 0) {
                    this.$service.web.createService(formData)
                        .then(res => {
                            if (res["code"] == 200) {
                                this.getWebList(this.targetGroup);
                                this.errText = "添加成功";
                    ...
                    this.$service.web.updateService(formData)
                        .then(res => {
                            if (res["code"] == 200) {
                                this.getWebList(this.targetGroup);
                                this.errText = "更新成功";
            ...
            getWebList(groupId) {
                ...
                this.$service.web.getWebListService(groupId, this.pagination.page, this.pagination.itemsPerPage)
                    .then(res => {
                        this.list = res['data'].data != null ? res['data'].data : [];
                        this.pagination.itemsLength = res.data.total;
                        this.pagination.page = res.data.page_index;
            ...
            getGroupList(teamId) {
                this.$service.group.getListByTypeService(teamId, 1)
                    .then(res => {
            ...
            redirectToOverview(item) {
                ...
                this.$router.push(`/user/web/${item.id}/overview`);
            },
            deleteWeb(id) {
                this.$service.web.deleteWebService(id)
                    .then(() => {
                        this.getWebList(this.targetGroup);
                    })
            },
            ...
            onDelete() {
                this.deleteWeb(this.onDeleteId);
                this.onDeleteId = "";
                this.dialog = false;
            }
        },
