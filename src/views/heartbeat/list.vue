        ...
        <v-dialog v-model="addDialogStatus" fullscreen hide-overlay transition="dialog-bottom-transition">
            <v-card>
                <v-toolbar dark class="deep-purple">
                    ...
                    <v-toolbar-title>心跳监控</v-toolbar-title>
                    <v-spacer></v-spacer>
                    <v-toolbar-items>
                        <v-btn dark text @click="createHeartbeat()">
                            确认保存
                        </v-btn>
                    </v-toolbar-items>
                </v-toolbar>
                <v-card-text>
                    <v-container>
                        <v-row>
                            ...
                            <v-col cols="12" sm="12">
                                <v-text-field v-model="path" label="请求地址" hide-details="auto"></v-text-field>
                            </v-col>
                            <v-col cols="12" sm="12">
...
<script>
    ...
    import {generateRdStr} from '../../utils/tools';
    ...
    export default {
        ...
        data: () => ({
            ...
            path:"",
            url: "https://api.pingsilo.com/monitor/heartbeat/",
            name: "",
            list: [],
            isLoading: true,
            isDisabled: true,
        ...
        methods: {
            getDetail(item) {
                this.$service.device.getDetailService(item.id, this.deviceType)
                    .then(res => {
                        ...
                        this.path=this.url+res['data'].token;
            ...
            pauseItem(item) {
                ...
                this.$service.heartbeat.pauseService(formData)
                    .then(() => {
            ...
            startMonitor(item) {
                const formData = new FormData();
                formData.append("id", item.id)
                this.$service.heartbeat.startMonitorService(formData)
                    .then(() => {
                        this.getList(this.targetGroup);
                    })
            },
            file(item) {
                const formData = new FormData();
                formData.append("id", item.id)
                this.$service.heartbeat.fileService(formData)
                    .then(() => {
                        this.getList(this.targetGroup);
                    })
            },
            getFiledList() {
                ...
                this.groupType = 2;
                this.$service.heartbeat.getFiledListService(this.pagination.page, this.pagination.itemsPerPage)
                    .then(res => {
            ...
            createHeartbeat() {
                ...
                formData.append("heartbeat_id", this.targetDevice);
                ...
                if (this.targetDevice == 0) {
                    this.$service.heartbeat.createService(formData)
                    .then(res => {
                        if (res["code"] == 200) {
                            this.getList(this.targetGroup);
                            this.errText = "添加成功";
                    ...
                    this.$service.heartbeat.updateService(formData)
                    .then(res => {
                        if (res["code"] == 200) {
                            this.getList(this.targetGroup);
                            this.errText = "更新成功";
            ...
            getList(groupId) {
                ...
                this.$service.heartbeat.getListService(groupId, this.pagination.page, this.pagination.itemsPerPage)
                    .then(res => {
                        this.list = res['data'].data != null ? res['data'].data : [];
                        this.pagination.itemsLength = res.data.total;
                        this.pagination.page = res.data.page_index;
            ...
            getGroupList(teamId) {
                this.$service.group.getListByTypeService(teamId, 6)
                    .then(res => {
            ...
            redirectToOverview(item) {
                ...
                this.$router.push(`/user/heartbeat/${item.id}/overview`);
            },            
            deleteDevice(id) {
                this.$service.heartbeat.deleteService(id)
                    .then(() => {
                        this.getList(this.targetGroup);
                    })
            },
        ...
        mounted() {
            this.token = generateRdStr(32).toLowerCase()+new Date().getTime();
            this.path=this.url+this.token;
            this.getTeamList();
        }
    }
</script>