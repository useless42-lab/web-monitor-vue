        ...
        <v-dialog v-model="addDialogStatus" fullscreen hide-overlay transition="dialog-bottom-transition">
            <v-card>
                <v-toolbar dark class="deep-purple">
                    ...
                    <v-toolbar-title>SteamGame服务器监控</v-toolbar-title>
                    <v-spacer></v-spacer>
                    <v-toolbar-items>
                        <v-btn dark text @click="createSteam()">
                            确认保存
                        </v-btn>
                    </v-toolbar-items>
                </v-toolbar>
                <v-card-text>
                    <v-container>
                        <v-row>
                            ...
                            <div>
                                <div>为了监控Steam游戏服务器，您需要一个Steam Api Key。</div>
                                <div>请自行申请<a href="https://steamcommunity.com/dev" target="_blank">Steam Api Key</a>并
                                    <router-link to="/user/setting/overview">填写</router-link>
                                </div>
                            </div>
                        </v-row>
                    </v-container>
    ...
    export default {
        ...
        methods: {
            ...
            pauseItem(item) {
                ...
                this.$service.steam.pauseService(formData)
                    .then(() => {
            ...
            startMonitor(item) {
                const formData = new FormData();
                formData.append("id", item.id)
                this.$service.steam.startMonitorService(formData)
                    .then(() => {
                        this.getList(this.targetGroup);
                    })
            },
            file(item) {
                const formData = new FormData();
                formData.append("id", item.id)
                this.$service.steam.fileService(formData)
                    .then(() => {
                        this.getList(this.targetGroup);
                    })
            },
            getFiledList() {
                ...
                this.groupType = 2;
                this.$service.steam.getFiledListService(this.pagination.page, this.pagination.itemsPerPage)
                    .then(res => {
            ...
            createSteam() {
                ...
                formData.append("steam_server_id", this.targetDevice);
                ...
                if (this.targetDevice == 0) {
                    this.$service.steam.createService(formData)
                        .then(res => {
                            if (res["code"] == 200) {
                                this.getList(this.targetGroup);
                                this.errText = "添加成功";
                    ...
                    this.$service.steam.updateService(formData)
                        .then(res => {
                            if (res["code"] == 200) {
                                this.getList(this.targetGroup);
                                this.errText = "更新成功";
            ...
            getList(groupId) {
                ...
                this.$service.steam.getListService(groupId, this.pagination.page, this.pagination.itemsPerPage)
                    .then(res => {
                        this.list = res['data'].data != null ? res['data'].data : [];
                        this.pagination.itemsLength = res.data.total;
                        this.pagination.page = res.data.page_index;
            ...
            getGroupList(teamId) {
                this.$service.group.getListByTypeService(teamId, 7)
                    .then(res => {
            ...
            redirectToOverview(item) {
                ...
                this.$router.push(`/user/steam/${item.id}/overview`);
            },
            deleteDevice(id) {
                this.$service.steam.deleteService(id)
                    .then(() => {
                        this.getList(this.targetGroup);
                    })
            },
