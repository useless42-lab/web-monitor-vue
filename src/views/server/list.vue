        ...
        <v-dialog v-model="addDialogStatus" fullscreen hide-overlay transition="dialog-bottom-transition">
            <v-card>
                <v-toolbar dark class="deep-purple">
                    ...
                    <v-toolbar-title>服务器监控</v-toolbar-title>
                    <v-spacer></v-spacer>
                    <v-toolbar-items>
                        <v-btn dark text @click="createServer()">
                            确认保存
                        </v-btn>
                    </v-toolbar-items>
                </v-toolbar>
                <v-card-text>
                    <v-container>
                        <v-row>
                            <v-col cols="12" sm="12">
                                <v-text-field v-model="name" label="服务器名称" hide-details="auto"></v-text-field>
                            </v-col>
                            <v-col cols="12" sm="12">
                            ...
                            <v-col cols="12" sm="12">
                                <v-text-field v-model="path" label="IP地址" hide-details="auto"></v-text-field>
                            </v-col>
                            <v-col cols="12" sm="12">
                ...
                <v-col cols="12" xl="10" lg="10" md="10" sm="9" xs="12">
                    <v-data-table :headers="headers" :items="list" :options.sync="pagination"
                        ...
                        <template v-slot:[`item.token`]="{item}">
                            <div>
                                {{item.token}}
                                <v-btn icon color="primary" @click="refreshToken(item.id)">
    ...
    export default {
        ...
        methods: {
            ...
            pauseItem(item) {
                ...
                this.$service.server.pauseService(formData)
                    .then(() => {
                        this.getServerList(this.targetGroup)
                    })
            },
            ...
            startMonitor(item) {
                const formData = new FormData();
                formData.append("id", item.id)
                this.$service.server.startMonitorService(formData)
                    .then(res => {
                        this.getServerList(this.targetGroup)
                    })
            },
            file(item) {
                const formData = new FormData();
                formData.append("id", item.id)
                this.$service.server.fileService(formData)
                    .then(res => {
                        this.getServerList(this.targetGroup)
                    })
            },
            getFiledList() {
                ...
                this.groupType = 2;
                this.$service.server.getFiledListService(this.pagination.page, this.pagination.itemsPerPage)
                    .then(res => {
            ...
            createServer() {
                ...
                formData.append("server_id", this.targetDevice);
                ...
                if (this.targetDevice == 0) {
                    this.$service.server.createService(formData)
                        .then(res => {
                            if (res["code"] == 200) {
                                this.getServerList(this.targetGroup);
                                this.errText = "添加成功";
                    ...
                    this.$service.server.updateService(formData)
                        .then(res => {
                            if (res["code"] == 200) {
                                this.getServerList(this.targetGroup);
                                this.errText = "更新成功";
            ...
            getServerList(groupId) {
                this.isLoading = true;
                this.isDisabled = true;
                this.groupType = 1;
                this.targetGroup = groupId;
                this.$service.server.getServerListService(groupId, this.pagination.page, this.pagination.itemsPerPage)
                    .then(res => {
                        this.list = res['data'].data != null ? res['data'].data : [];
                        this.pagination.itemsLength = res.data.total;
                        this.pagination.page = res.data.page_index;
            ...
            getGroupList(teamId) {
                this.$service.group.getListByTypeService(teamId, 2)
                    .then(res => {
            ...
            redirectToOverview(item) {
                ...
                this.$router.push(`/user/server/${item.id}/overview`)
            },
            refreshToken(serverId) {
                const formData = new FormData();
                formData.append("server_id", serverId)
                this.$service.server.refreshTokenService(formData)
                    .then(() => {
                        this.getServerList(this.targetGroup);
                    })
            },
            deleteServer(serverId) {
                this.$service.server.deleteServerService(serverId)
                    .then(() => {
                        this.getServerList(this.targetGroup);
                    })
            },
            ...
            onDelete() {
                this.deleteServer(this.onDeleteId);
                this.onDeleteId = "";
                this.dialog = false;
            }
        },
