        ...
        <v-dialog v-model="addDialogStatus" fullscreen hide-overlay transition="dialog-bottom-transition">
            <v-card>
                <v-toolbar dark class="deep-purple">
                    ...
                    <v-toolbar-title>DNS监控</v-toolbar-title>
                    <v-spacer></v-spacer>
                    <v-toolbar-items>
                        <v-btn dark text @click="createDns()">
                            确认保存
                        </v-btn>
                    </v-toolbar-items>
                </v-toolbar>
                <v-card-text>
                    <v-container>
                        <v-row>
                            <v-col cols="12" sm="12">
                                <v-text-field v-model="name" label="名称" hide-details="auto"></v-text-field>
                            </v-col>
                            <v-col cols="12" sm="12">
                                <v-text-field v-model="url" label="主机名" hide-details="auto"></v-text-field>
                            </v-col>
                            <v-col cols="12" sm="12">
                                <v-text-field v-model="dnsServer" label="解析服务器" hide-details="auto"></v-text-field>
                            </v-col>
                            <v-col cols="12" sm="12">
                                <v-select v-model="targetDnsType" :items="dnsTypeData" item-text="label"
                                    item-value="value" label="资源类型记录">
                                </v-select>
                            </v-col>
                            <v-col cols="12" sm="12">
    ...
    export default {
        ...
        methods: {
            getDetail(item) {
                this.$service.device.getDetailService(item.id, this.deviceType)
                    .then(res => {
                        ...
                        this.targetDnsType = res['data'].dns_type;
                        this.dnsServer = res['data'].dns_server;
            ...
            pauseItem(item) {
                ...
                this.$service.dns.pauseService(formData)
                    .then(() => {
            ...
            startMonitor(item) {
                const formData = new FormData();
                formData.append("id", item.id)
                this.$service.dns.startMonitorService(formData)
                    .then(() => {
                        this.getList(this.targetGroup);
                    })
            },
            file(item) {
                const formData = new FormData();
                formData.append("id", item.id)
                this.$service.dns.fileService(formData)
                    .then(() => {
                        this.getList(this.targetGroup);
                    })
            },
            getFiledList() {
                ...
                this.groupType = 2;
                this.$service.dns.getFiledListService(this.pagination.page, this.pagination.itemsPerPage)
                    .then(res => {
            ...
            createDns() {
                ...
                formData.append("path", this.url);
                formData.append("dns_type", this.targetDnsType);
                formData.append("dns_server", this.dnsServer);
                ...
                formData.append("dns_id", this.targetDevice);
                ...
                if (this.targetDevice == 0) {
                    this.$service.dns.createService(formData)
                        .then(res => {
                            if (res["code"] == 200) {
                                this.getList(this.targetGroup);
                                this.errText = "添加成功";
                     ...
                     this.$service.dns.updateService(formData)
                        .then(res => {
                            if (res["code"] == 200) {
                                this.getList(this.targetGroup);
                                this.errText = "更新成功";
            ...
            getList(groupId) {
                ...
                this.$service.dns.getListService(groupId, this.pagination.page, this.pagination.itemsPerPage)
                    .then(res => {
                        this.list = res['data'].data != null ? res['data'].data : [];
                        this.pagination.itemsLength = res.data.total;
                        this.pagination.page = res.data.page_index;
            ...
            getGroupList(teamId) {
                this.$service.group.getListByTypeService(teamId, 5)
                    .then(res => {
            ...
            deleteDevice(id) {
                this.$service.dns.deleteService(id)
                    .then(() => {
                        this.getList(this.targetGroup);
                    })
            },
